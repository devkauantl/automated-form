...
def openInterface():
        ...
        if comboboxType.get() == " Aprovado":
            if os.path.exists(fileAprovado):
                with open(fileAprovado, 'a') as file:
                    file.write("|-----------| APROVADO |-----------|\n| ID: {}\n| Nick: {}\n| Discord: {}\n| WhatsApp: {}\n|----------------------------------|\n\n"
                                  .format(entryForm.get(), entryNick.get(), entryDiscord.get(), entryWhats.get()))
            else:
    ...
    def buttonClearResponse():
        ...
        entryNick.delete(0, customtkinter.END)  # Limpa o conteúdo da entrada
        entryNick.selection_clear()  # Remove a seleção do texto
        entryNick.configure(placeholder_text="Nick")  # Garante que o placeholder seja mostrado novamente
        root.focus()
    ...
    # Criando o campo de entrada para o "Nick"
    entryNick = customtkinter.CTkEntry(master=labelBackground, width=385, height=35, placeholder_text="Nick", bg_color=labelColor, corner_radius=0, font=('Century Gothic', 15))
    entryNick.place(x=380, y=220 + 35)  # Posicionando o campo de entrada do Nick
